...
<head>
    ...
    <style>
        ...
        th{background: #ff2b50; color: white; }
...
<!-- Форма выбора вида сортировки -->
            ...
            <div class="container">
                <div class="row row-cols-4">
                    <div class="col" style="display: flex; align-items: center"><label for="sortValue" style="font-size: 16px">Сортировать по: </label></div>
                    <div class="col"> <select class="form-select" name="sort_value" id="sortValue">
                            <option value="id" selected>Номеру</option>
                            <option value="id_worker">Работнику (сначала)</option>
                            <option value="id_worker DESC">Работнику (с конца)</option>
                            <option value="name">Месту</option>
                        </select>
                    </div>
                    <div class="col"><input class="btn" style="background-color: #ff2b50; color: #ffffff; font-weight: bold" type="submit" value="Применить"></div>
                </div>
            </div>
            </form>
...
try {
    ...
    $sql = "SELECT id, img, name, id_worker FROM nums ORDER BY $sort_value";
    // Подготовка запроса
    ...
    echo "<table><tr><th>ID</th><th>img</th><th>Место</th><th>Номер работника</th></tr>";
    foreach ($result_array as $result_row) {
        echo "<tr>";
        echo "<td>" . $result_row["id"] . "</td>";
        echo "<td>" . $result_row["img"] . "</td>";
        echo "<td>" . $result_row["name"] . "</td>";
        echo "<td>" . $result_row["id_worker"] . "</td>";
        echo "</tr>";
    }
    echo "</table>";
}
